      ...
      <AnimatePresence>
        {open && (
          <motion.ul
            ...
            className="absolute right-0 mt-1.5 w-32 
                       bg-white dark:bg-neutral-900 
                       border border-neutral-200 dark:border-neutral-800 
                       rounded-xl shadow-xl dark:shadow-2xl dark:shadow-black/50
                       overflow-hidden z-50 list-none p-1"
          >
            {routing.locales.map((loc) => {
              ...
              return (
                <li key={loc}>
                  <button
                    onClick={() => select(loc)}
                    className={`w-full flex items-center justify-between gap-2 px-3 py-2 text-sm rounded-lg transition-all duration-150 cursor-pointer
                      ${isActive
                        ? 'text-neutral-900 dark:text-white font-semibold bg-neutral-100 dark:bg-neutral-800'
                        : 'text-neutral-600 dark:text-neutral-400 hover:bg-neutral-100/70 dark:hover:bg-neutral-800/70 hover:text-neutral-900 dark:hover:text-white'
                      }`}
                  >
